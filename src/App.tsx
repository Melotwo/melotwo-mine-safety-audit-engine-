...
export { KlaviyoOutreachCenter } from './components/KlaviyoOutreachCenter';

// Inline KlaviyoOutreachCenter re-exported from ./components/KlaviyoOutreachCenter
  ...
  if (currentPage === 'home' || currentPage === 'solutions') {
    ...
    return (
      <ShiftHandoverAssistant
        onSyncToLedger={async (recordData) => {
          try {
            const saved = localStorage.getItem('melotwo_sandbox_logs');
            let currentLogs: any[] = [];
            if (saved) {
            ...
            const updated = [recordData, ...currentLogs];
            localStorage.setItem('melotwo_sandbox_logs', JSON.stringify(updated));
          } catch (err) {
