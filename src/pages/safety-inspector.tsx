...
export const SafetyInspectorPage: React.FC<SafetyInspectorPageProps> = ({ setPage }) => {
    ...
    const processDocumentText = async (text: string) => {
        ...
        try {
            ...
            if (!response.ok) {
                throw new Error(`Server scanner error: ${response.statusText}`);
            }
            ...
            setScanError('Failed to automatically parse document. Please enter details manually or retry.');
        } finally {
    ...
    const runAudit = async (isOperationalAudit: boolean) => {
        ...
        try {
            ...
            if (!res.ok) {
                throw new Error(`Draft generation failed: ${res.statusText}`);
            }
            ...
            setError(err.message || 'Failed to generate assessment draft.');
            setResponse(null);
        } finally {
            setLoading(false);
        }
