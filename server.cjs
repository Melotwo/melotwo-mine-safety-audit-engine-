...
function getSANSFallbackAudit(params) {
  const { miningSector, depthLevel = 1e3, environmentHazards = [], currentPPE = {} } = params;
  ...
  let requiredArcMin = "8 cal/cm\xB2";
  ...
  if (environmentHazards.includes("Electric Arc")) {
    if (arcRating === "No rating" || parseInt(arcRating) < 8) {
      ...
      remediation.push("Upgrade all electrical switching and heavy equipment handling staff to certified SANS 724 garments with Arc Thermal Performance Value (ATPV) exceeding 8 cal/cm\xB2 (HRC 2).");
  ...
  const historyLog = {
    id: `AUD-${Math.floor(1e5 + Math.random() * 9e5)}`,
...
function getDailyShiftFallbackBriefing(params) {
  ...
  return {
    ...
    complianceActionPlan: {
      ...
      requiredMaterialSpecifications: {
        ...
        minimumPerformanceRating: "8.4 cal/cm\xB2",
...
app.post("/api/audit", async (req, res) => {
  try {
    ...
    try {
      ...
      if (dailyShiftCheck) {
        const systemPrompt2 = `You are a high-engagement, safety-focused SHEQ shift officer for Melotwo Mine Safety.
...
Output strictly valid JSON matching this schema. Do not include markdown ticks or conversational text:
{
  ...
  "vendorMatchingCriteria": { "targetSupplierCategory": "Outfitters", "bulkOrderSpecsSummary": "Daily check" }
        ...
        const promptMsg2 = `Generate Daily Shift Safety Briefing for:
...
- Staffing Headcount: ${params.headcount || 120} workers`;
        ...
        const cleanedJson2 = rawText2.replace(/```json/gi, "").replace(/```/g, "").trim();
        const parsedBriefing = JSON.parse(cleanedJson2);
